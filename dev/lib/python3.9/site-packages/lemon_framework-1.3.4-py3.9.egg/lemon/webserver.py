...
try:
    ...
    lemon.libs.logging.error("No ssl")
...
async def handle_client(reader,writer):
    ...
    timed_out = 0
    try:
        dateandtime = lemon.libs.Date.httpdate()
        ...
        if run:
                ...
                if  current_http_status != 1:
                    if "\r\n\r\n" in http_request["data"].decode("utf-8",errors="ignore"):
                        if  re.findall(r"Host:\s(.*)", http_request["data"].decode("utf-8",errors="ignore")) != []:
                            ...
                            bad_request = 1
                            lemon.libs.logging.error("[!] Bad Request\n")
                            break
                if current_http_status == 1:
            ...
            if bad_request == 1:
                try:
                    writer.write(lemon.libs.create_http.create_error("Bad Request","400"))
                    await writer.drain()
                    writer.close()
                except Exception as e:
                    lemon.libs.logging.error(e)
            elif timed_out == 1:
                try:
                    writer.close()
                except Exception as e:
                ...
                if headers.decode("utf-8",errors="ignore").replace(" ","") != "":
                    ...
                    request_object = lemon.libs.handleHttp.http(headers)
                    if request_object.headers["Host"].split(":")[0] not in config.config.ALLOWED_HOSTS:
                        writer.write(lemon.libs.create_http.create_error("Bad Request, Host header incorrect","400"))
                        await writer.drain()
                        writer.close()
                    else:                
                        print("Request: "+ request_object.page,end = "")
                        print(" ",end="")
                        object = lemon.libs.HttpObject.HttpObject(request_object.page,request_object.GET,request_object.POST,request_object.cookies,request_object.request_type)
                        object.status = "200"
        ...
        if time_took > 3.0:
            time_message = "{}{}{}".format(lemon.libs.colors.colors.fg.red, time_took,lemon.libs.colors.colors.reset)
        else:
            time_message = "{}{}{}".format(lemon.libs.colors.colors.fg.green,time_took,lemon.libs.colors.colors.reset)
    
        lemon.libs.logging.log("It took ")
        lemon.libs.logging.log(time_message)
        lemon.libs.logging.log(" seconds to proccess and return request\n")
        try:
        ...
        print(e)
        writer.close()
        lemon.libs.logging.error("A error has occured while handling request\n")
...
def start_non_ssl_server():
    try:
        ...
        if sock_error.errno == 98:
            lemon.libs.logging.error("Socket already in use. Exiting...\n")
            sys.exit(0)
...
def start_ssl_server(ssl_context):
    try:
        ...
        The default value is 1000. so the default workers is 1000.
        """
        loop1.set_default_executor(concurrent.futures.ThreadPoolExecutor(config.config.ASYNCIO_MAX_WORKERS))
        """
        Starts the http server.
        """
        loop1.create_task(asyncio.start_server(handle_client, HOST, config.config.SSL_PORT, ssl= ssl_context))
        """
        this makes the server run forever
        """
        loop1.run_forever()
        ...
        lemon.libs.logging.error("A error has occured while creating server socket\n")
        lemon.libs.logging.log(e)
...
def server_main():
    ...
    if config.config.NORMAL_SERVER:
        try:
            ...
            running_threads.append(normal_thread)
            normal_thread.start()
        except KeyboardInterrupt:
            sys.exit(0)
        except OSError as sock_error:
    ...
    if config.config.SSL:
        ...
        try:
            ...
            running_threads.append(ssl_thread)
            ssl_thread.start()
        except KeyboardInterrupt:
            sys.exit(0)
        except OSError as sock_error:
